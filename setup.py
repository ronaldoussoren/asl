...
def _extract_extension(cfg, extname):
    ...
    # XXX: look up other keyword arguments for Extension
    _opt_value(cfg, 'extra_compile_args', sect, 'extra_compile_args', _as_list)
    _opt_value(cfg, 'extra_link_args', sect, 'extra_link_args', _as_list)
...
if 'ext_modules' in metadata:
    metadata['ext_modules'] = [Extension(*args, **kwds) for (args, kwds) in metadata['ext_modules']]
